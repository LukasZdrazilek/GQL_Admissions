...
async def get_context(request: Request):
    ...
    # i = Item(query = "")
    # logging.debug(f"before sentinel current user is {request.scope.get('user', None)}")
    # await sentinel(request, i)
    # logging.debug(f"after sentinel current user is {request.scope.get('user', None)}")

    result = {**context}
    result["request"] = request
    # result["user"] = request.scope.get("user", None)
    # logging.debug(f"context created {result}")
    return result
...
MountGuardedGQL(app=app, mountpoint="/gql", schema=schema, get_context=get_context, DEMO=DEMO)
